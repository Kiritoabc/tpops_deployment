...
从 **`POST /api/deployment/tasks/`** 到 **`run_task_async` → `_run_task` → `_run_task_body`**，以及 `_sync_pkgs_to_remote`、写配置、`run_remote_command`、`_emit`、WebSocket 的完整顺序，见：

**[`../deployment-task-execution-flow.md`](../deployment-task-execution-flow.md)**
...
**函数级逐步说明：** [部署任务执行流程](../deployment-task-execution-flow.md)
