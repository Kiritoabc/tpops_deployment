...
> **分支**：`go-dev` — Go 相关实现与文档均在此分支推进。  
> **状态**：`go/` 已落地骨架与部分 API；Runner 全量流式、日志 WS、包上传等待办。

## 1. 背景与目标

- 仓库内另有历史 **Web + API + 实时通道 + SQLite + 前端 SPA** 实现（根目录文档见 `AGENTS.md`、`docs/PROJECT_GUIDE.md`）。
- 目标在 **`go-dev`** 探索 **轻量级** 替代：**单二进制、SQLite 单文件、低依赖**，尽量复用现有 **REST / WebSocket 契约** 与前端字段，降低切换成本。
...
- 多区域高可用、K8s Operator。
- 与**其他进程**对 **同一 SQLite 文件并发写**（迁移期禁止，见风险）。
...
| SSH/SFTP | **`golang.org/x/crypto/ssh`** + **`github.com/pkg/sftp`** | 对齐常见 SSH/SFTP 能力 |
| 校验 | **`go-playground/validator/v10`** | 请求 DTO |
| 日志 | **`go.uber.org/zap`** 或 **zerolog** | 结构化 |
| 静态资源 | **`embed.FS`** + Gin 静态路由 | 托管 SPA 或静态页 |
...
在 `go-dev` 上维护 **独立 Go 模块**（`go/` 目录），与仓库根目录其他工作流隔离：
...
go/
  ...
  internal/
    ...
    handler/ws/              # WS + Hub
    ssh/
    manifest/                # 与既有 manifest 解析行为对齐（测试驱动）
    crypto/                  # 主机密钥加解密（Fernet 兼容或迁移说明）
  web/static/                # embed 静态前端（可选）
```

CI 可增加 `go test ./...`（在 `go/` 目录下执行）。
...
4. **备份**：文件级备份 + WAL checkpoint 说明；文档写明「不建议两个服务进程同时写同一 `.db`」。

## 6. API 与 WebSocket 衔接

- **REST**：保持 `/api/auth/`、`/api/hosts/`、`/api/deployment/tasks/`、`/api/packages/` 等前缀；差异在 `plan` 末尾「契约差异表」维护。
- **WebSocket**：与现网一致的路径与 query（如 `token`）；消息类型：`hello`、`phase`、`log`、`manifest`、`manifest_wait`、`status`、`done` 等（与既有 runner、通道消费者约定对齐）。
...
| manifest 与参考实现不一致 | 同输入 YAML 双端 JSON 对比测试；差异文档化 |
| 双栈共 SQLite | 禁止双写；迁移窗口只读或导出再切换 |

## 10. 待产品 / 技术确认

- Go **目标版本**：`go/` 已锁定 **1.18**（`go.mod` 中 `go 1.18`）；CI 建议固定 `GOTOOLCHAIN=go1.18.x` 或镜像内 Go 1.18，避免在较新工具链上“能编过”但依赖的 `go` directive 实际要求更高版本。
...
- **2026-04**：落地 `go/` 模块骨架：`cmd/server`、Gin、`goose` 迁移、`SQLite`、JWT、`/api/auth/login|register|profile`、`/api/hosts/`、`/api/deployment/tasks/`（详见 `go/README.md`）。**未实现**：WS 全量、runner、manifest、包管理。
- **2026-04**：增加 `manifest` 包（单/多节点合并与参考解析对齐）、`GET .../manifest_snapshot/`、`GET .../tasks/:id/`、WebSocket `/ws/deploy/:id/?token=`（hello）；Fernet 解密主机凭证；模块路径 `tpops_deployment`、`embed` 静态页。**未实现**：runner 实时流、包上传、log tail WS。
