...
- **2026-04**：Runner：**SFTP 同步**选中制品到远端 `<部署根>/pkgs/`；**appctl 封装**（install/upgrade/uninstall_all/precheck_*）；**user_edit** SFTP 下发到 `config/user_edit.conf`（或可配置路径）。**待办**：登录限流、多节点并行同步。
