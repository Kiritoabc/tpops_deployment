...
5. **③** 再执行所选 `appctl.sh` 操作。WebSocket 的 appctl 日志区会打出 `[步骤]`、`[tpops-media]`、`[pkgs]` 等前缀行，便于对照传包与解压。任务还会在服务端追加写入 `logs/deployment_tasks/task_<id>.log`（可通过 `DEPLOYMENT_TASK_LOG_DIR` 覆盖目录）；部署监控页在传包阶段会显示上传进度条与解压/移动提示。
