...
- **install / upgrade**：先等待 `config/gaussdb/init_manifest_successful`（或同目录 `.txt` / `config/` 下备选）出现后，再轮询 `manifest.yaml` 与 `manifest_{node_ip}.yaml`（来自 `user_edit` 的 node1/2/3_ip）并**聚合**展示。
