...
- 安装前置检查：`bash appctl.sh precheck install <组件>`（如 `gaussdb`）
- 升级前置检查：`bash appctl.sh precheck upgrade <组件>`
- 安装：`bash appctl.sh install` 或 `bash appctl.sh install <参数>`（按脚本需要）
