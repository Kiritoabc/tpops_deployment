...
| **远端目录** | **扁平**：`<部署根>/pkgs/`，即 **`{docker_service_root}/pkgs/`**（如 `/data/docker-service/pkgs/`）。**不按版本建子目录**；远端落盘文件名与「平台存储文件名 / 展示用原名」规则见下「同名覆盖」。 |
| **与 appctl** | 同步完包后**照常**执行现有 `sh appctl.sh ...`；**不把包路径拼进 appctl 命令行**，由现场脚本从 `pkgs/` 取用。 |

### 同名包覆盖策略

1. **平台库（某版本下多条上传记录）**  
   - **同一版本（Release）内**：若新上传文件与已有 **存储文件名**（或规范化后的 `remote_basename`）相同 → **视为同一介质更新**：覆盖平台侧存储对象、更新元数据（大小、SHA256、上传时间、操作人）；**不保留两条同名记录**。  
   - **不同版本之间**：允许同名（例如两个版本里都有 `foo.tar.gz`）；下发到远端时仍落到**同一扁平目录** `pkgs/`，故 **后下发的任务覆盖远端同名文件**（见下条）。

2. **下发到远端 `pkgs/`**  
   - 同步时使用确定的目标文件名（建议与平台 **存储名 / `remote_basename`** 一致）。  
   - 若远端已存在同名文件：**直接覆盖**（实现上用「先写临时文件再 `rename` 替换」保证原子性，与现有 SFTP 写配置一致）。  
   - 日志 / WebSocket 中可打一行：`pkgs/<name> replaced` 便于审计。

3. **路径安全**  
   - 禁止 `..`、绝对路径、路径分隔符混入文件名；规范化后仅允许单段文件名。
