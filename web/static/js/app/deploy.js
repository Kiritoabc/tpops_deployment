  ...
  createDeployModule(refs, shared, pageState, auth, hosts, packages) {
    ...
    const openSocket = (taskId, opts = {}) => {
      ...
      socket.onmessage = (ev) => {
        ...
        if (msg.type === 'log') {
          if (msg.data) logText.value += msg.data;
          else if (msg.line) logText.value += msg.line + '\n';
        }
        if (msg.type === 'manifest') {
          const pl = msg.data || msg.manifest;
          if (pl) applyManifestPayload(pl);
        }
        if (msg.type === 'manifest_wait') {
          let tip = (msg.message || msg.reason || '') + (msg.paths ? ' — ' + msg.paths.join(', ') : '');
          if (msg.details && msg.details.length) tip += ' | ' + msg.details.join(' | ');
          phaseHint.value = tip;
        }
        if (msg.type === 'status') {
          const st = msg.data != null ? msg.data : msg.status;
          taskStatus.value = st;
          applyLiveTaskPatch({ status: st });
          if (st === 'running') {
            flowS1.value = 'active';
